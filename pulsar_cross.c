...
void Display(void)
{
   ...
   if (stereo) {
      ...
      glMatrixMode(GL_MODELVIEW);
      //pseudo (GL_BACK_RIGHT);
      glViewport(0,0,(GLsizei)camera.screenwidth/2,(GLsizei)camera.screenheight/2);
...
{
   /*int boardDimen=8;
   float boardSize=camera.screenwidth/4*5;
   float tileSize=boardSize/boardDimen;
   float cx= camera.vp.x + camera.vd.x;
   float cy= camera.vp.y + camera.vd.y;
   float cz= camera.vp.z + camera.vd.z;
   glPushMatrix();
   glTranslatef(0,0,-400);
   glBegin(GL_QUADS);
   glColor4f(1,1,1,1);
   glNormal3f(0,0,1);
   glVertex3f(-boardSize/2,boardSize/2,0);
   glVertex3f(-boardSize/2,-boardSize/2,0);
   glVertex3f(boardSize/2,-boardSize/2,0);
   glVertex3f(boardSize/2,boardSize/2,0);
   glEnd();
   ...
   glPushMatrix();
   glTranslatef(-boardSize/2,boardSize/2,1);
   for(int i=0;i<boardDimen;i++){
      glPushMatrix();
      if(i%2==0){
         glTranslatef(tileSize,0,0);
      }
      for(int j=0;j<boardDimen/2;j++){
         glBegin(GL_QUADS);
         glColor4f(1,0,0,1);
         glNormal3f(0,0,1);
         glVertex3f(0,0,0);
...
/*
   camera.vp.x = 0;//camera.focallength; 
   camera.vp.y = 0; 
